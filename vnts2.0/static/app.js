...
function bindThemeMediaQuery() {
  ...
  state.themeMediaQuery = window.matchMedia(SYSTEM_THEME_MEDIA);
  ...
  if (typeof state.themeMediaQuery.addEventListener === "function") {
    state.themeMediaQuery.addEventListener("change", syncSystemTheme);
    return;
  }
  if (typeof state.themeMediaQuery.addListener === "function") {
    state.themeMediaQuery.addListener(syncSystemTheme);
  }
}
...
function applyThemePreference(themePreference, options = {}) {
  const normalizedPreference = themePreference === "dark" || themePreference === "light"
    ? themePreference
    : "system";
  const resolvedTheme = normalizedPreference === "system" ? resolveSystemTheme() : normalizedPreference;
  state.themePreference = normalizedPreference;
  state.theme = resolvedTheme;
  document.documentElement.setAttribute("data-theme", resolvedTheme);
  document.documentElement.setAttribute("data-theme-source", normalizedPreference);
  updateThemeUi();
...
function updateThemeUi() {
  ...
  dom.themeModeText.textContent = state.themePreference === "system"
    ? `跟随系统 · ${dark ? "深色" : "浅色"}`
    : dark
      ? "手动深色"
      : "手动浅色";
...
function renderStatus(status) {
  ...
  dom.metricCpu.textContent = status.process?.cpu_percent ? `${status.process.cpu_percent}%` : "-";
  dom.metricMemory.textContent = status.process?.memory_percent ? `${status.process.memory_percent}%` : "-";
