...
export async function renderDashboard(root) {
  ...
  const professional = state.settings?.experience_mode === "professional";
  root.innerHTML = `
    <section class="section-header"><div><h2>你好，${escapeHtml(config.device_name || "Linux 设备")}</h2><p>${diagnostic(status)}</p></div><div class="actions"><button class="button outline" data-jump="configs">添加配置</button><label class="experience-toggle"><span>${professional ? "专业模式" : "极简模式"}</span><span class="switch"><input id="experienceModeToggle" type="checkbox" ${professional ? "checked" : ""} aria-label="切换极简模式和专业模式"><i></i></span></label><button class="icon-button" type="button" data-jump="settings" title="设置" aria-label="设置">⚙</button><button class="button ${running ? "danger" : "primary"}" id="connectionButton">${running ? "断开连接" : "启动连接"}</button></div></section>
    <section class="dashboard-hero">
      <article class="panel connection-overview actionable" id="connectionCard" role="button" tabindex="0" aria-label="${running ? "断开全部服务器" : "链接全部添加的服务器"}"><div class="panel-header"><div><span class="badge ${running ? "success" : ""}">${running ? phaseLabel(status.phase) : "未连接"}</span><h3>虚拟网络</h3></div><span class="network-orb ${running ? "active" : ""}">⌁</span></div><strong class="virtual-ip">${running ? escapeHtml(status.virtual_ip || "—") : "未连接"}</strong><p>${running ? escapeHtml(status.virtual_network || "尚未分配虚拟网络") : "点击卡片即可链接全部添加的服务器"}</p><dl class="compact-details"><div><dt>设备名称</dt><dd>${escapeHtml(config.device_name || "—")}</dd></div><div><dt>服务器</dt><dd>${escapeHtml(status.connected_server || config.server_addresses?.[0] || "—")}</dd></div><div><dt>协议</dt><dd>${escapeHtml((status.connected_server || config.server_addresses?.[0] || "—").split("://")[0].toUpperCase())}</dd></div></dl></article>
    ...
    ${professional ? `<section class="dashboard-lower">
  ...
  const changeConnection = async () => {
    ...
    const connectionButton = root.querySelector("#connectionButton");
    const connectionCard = root.querySelector("#connectionCard");
    connectionButton.disabled = true;
    connectionCard.setAttribute("aria-busy", "true");
    try { await post(running ? "/stop" : "/start"); toast(running ? "网络已断开" : "网络正在启动"); }
    catch (error) { toast(error.message, "error"); }
    finally {
      connectionPending = false;
      connectionButton.disabled = false;
      connectionCard.removeAttribute("aria-busy");
      document.dispatchEvent(new CustomEvent("vntc:refresh"));
    }
  };
  root.querySelector("#connectionButton").addEventListener("click", changeConnection);
  root.querySelector("#connectionCard").addEventListener("click", changeConnection);
  root.querySelector("#connectionCard").addEventListener("keydown", (event) => {
    if (event.key !== "Enter" && event.key !== " ") return;
    event.preventDefault();
    changeConnection();
  });
  root.querySelector("#experienceModeToggle").addEventListener("change", async (event) => {
    const nextMode = event.currentTarget.checked ? "professional" : "minimal";
    if (nextMode === "professional" && !confirm("除非你知道自己在做什么，否则最纯粹的虚拟组网体验更适合你。是否进入专业模式？")) {
      event.currentTarget.checked = false;
      return;
    }
    const candidate = { ...state.settings, experience_mode: nextMode };
    try {
      await put("/settings", candidate);
      state.settings = candidate;
      document.dispatchEvent(new CustomEvent("vntc:settings"));
      toast(`已切换到${nextMode === "professional" ? "专业模式" : "极简模式"}`);
      renderDashboard(root);
    } catch (error) {
      event.currentTarget.checked = professional;
