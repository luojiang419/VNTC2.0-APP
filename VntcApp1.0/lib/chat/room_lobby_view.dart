...
class _RoomLobbyViewState extends State<RoomLobbyView> {
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            Text(
              '网络 {networkCount} 个 · 在线设备 {deviceCount} 个'.tr({
                'networkCount': _connectedNetworkKeys.length,
                'deviceCount': _onlinePeers.length,
              }),
  ...
  Future<void> _showPeerContextMenu(
    ...
    final selected = await showMenu<String>(
      ...
      items: [
        PopupMenuItem(value: 'chat', child: Text('发起私信'.tr())),
        PopupMenuItem(value: 'request_control', child: Text('请求控制'.tr())),
        PopupMenuItem(value: 'invite_control', child: Text('邀请控制'.tr())),
        if (friendStatus == ChatFriendStatus.stranger)
          PopupMenuItem(value: 'friend', child: Text('加好友'.tr())),
        PopupMenuItem(value: 'remark', child: Text('设置备注'.tr())),
        PopupMenuItem(value: 'block', child: Text('拉黑'.tr())),
      ],
    );
    if (selected == null) {
      return;
