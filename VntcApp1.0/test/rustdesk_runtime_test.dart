...
''';
  ...
  group('ensureHostReady', () {
    test('使用 bundled runtime 写入直连配置并只拉起内置主进程 host', () async {
      ...
      await runtime.ensureHostReady(listenPort: 21118);
      ...
      expect(config, contains("approve-mode = 'click'"));
      expect(
          config, contains("verification-method = 'use-temporary-password'"));
    ...
    test('bundled runtime 缺失时不会回退到系统安装版', () async {
      ...
      await expectLater(
        () => runtime.ensureHostReady(listenPort: 21118),
  ...
  group('openRemoteDesktop', () {
    test('控制端只使用 bundled runtime 建立连接', () async {
      final startedExecutables = <String>[];
      final startedArgs = <List<String>>[];
      ...
      await runtime.openRemoteDesktop(targetAddress: '10.0.0.2:21118');
    ...
    test('bundled runtime 缺失时不会回退到系统安装版建立连接', () async {
      ...
      await expectLater(
        () => runtime.openRemoteDesktop(targetAddress: '10.0.0.2:21118'),
