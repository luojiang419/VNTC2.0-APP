...
if [[ "${1:-}" == "--no-advance" ]]; then
  advance_version=false
elif [[ $# -gt 0 ]]; then
  echo "用法：scripts/build_linux_webui.sh [--no-advance]" >&2
  exit 1
fi
...
crate_version="$(sed -n 's/^version = "\([0-9][0-9.]*\)"/\1/p' "$crate_dir/Cargo.toml" | head -n 1)"
if [[ "$crate_version" != "$version.0" ]]; then
  echo "Cargo 版本 $crate_version 与构建版本 $version 不一致" >&2
  exit 1
fi
...
echo "[2/6] 运行测试"
cargo test --manifest-path "$crate_dir/Cargo.toml" --locked --all-targets

echo "[3/6] 编译 Release"
cargo build --manifest-path "$crate_dir/Cargo.toml" --release --locked
...
echo "[5/6] 校验二进制与安装包内容"
"$stage_dir/vntc-linux-webui" --help >/dev/null
