...
- `pubspec.yaml`：`version: 1.0.0+43`
- `AppUpdateConfig.currentVersion`：`1.0.0.43`
- `AppUpdateConfig.currentVersionTag`：`v1.0.0.43`
- Inno Setup：`#define MyAppVersion "1.0.0.43"`
...
| 当前版本 | `1.0.0.43` | 应用内比较版本 |
| 当前 tag | `v1.0.0.43` | GitHub Release tag |
...
- Flutter 版本写法：`1.0.0+43`
- 应用内版本写法：`1.0.0.43`
- GitHub tag 写法：`v1.0.0.43`
- 安装包版本写法：`1.0.0.43`

对应关系：

```text
pubspec.yaml: 1.0.0+43
AppUpdateConfig.currentVersion: 1.0.0.43
AppUpdateConfig.currentVersionTag: v1.0.0.43
installer MyAppVersion: 1.0.0.43
Release tag: v1.0.0.43
Asset: StoryboardGridApp-Setup-1.0.0.43.exe
```
...
static const currentVersion = '1.0.0.44';
static const currentVersionTag = 'v1.0.0.44';
```

```iss
; installer/storyboard_grid_app.iss
#define MyAppVersion "1.0.0.44"
```
...
```text
dist/installer/StoryboardGridApp-Setup-1.0.0.44.exe
```

校验文件大小与 SHA256：

```powershell
Get-Item "dist\installer\StoryboardGridApp-Setup-1.0.0.44.exe" | Select-Object Name,Length,LastWriteTime
Get-FileHash "dist\installer\StoryboardGridApp-Setup-1.0.0.44.exe" -Algorithm SHA256
```
...
```powershell
gh release view "v1.0.0.44" --repo "luojiang419/storyboard-grid-app-releases"
```

如果不存在，再创建：

```powershell
gh release create "v1.0.0.44" `
  "dist\installer\StoryboardGridApp-Setup-1.0.0.44.exe" `
...
- tag：`v1.0.0.44`
- release 标题：`故事板 v1.0.0.44`
- 上传资产：`StoryboardGridApp-Setup-1.0.0.44.exe`
- 非 draft。
...
draft=False
prerelease=False
asset_name=StoryboardGridApp-Setup-1.0.0.44.exe
asset_url=https://github.com/luojiang419/storyboard-grid-app-releases/releases/download/v1.0.0.44/StoryboardGridApp-Setup-1.0.0.44.exe
```
...
以下以从 `v1.0.0.43` 发布到 `v1.0.0.44` 为例。
